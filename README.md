...
Fill & sign now previews the PDF in-page: draw, type or upload a visual signature; drag, resize and place it on the current, selected or all pages before export. It also supports text fields, checkboxes, dropdowns, option lists and radio groups. Rotated pages cannot yet receive visual signatures. Images to PDF supports manual page order, orientation, margins, paper size and image quality. PDF.js renders the document locally on the dedicated page; PDF and GIF libraries load only when their respective tools open, which keeps the rest of Tools faster.
...
- **Not tested yet at all**: on-device AI (Summarizer, Rewriter, Proofreader, Translator), screen recorder, replay buffer, webcam bubble, video to GIF, new standalone PDF Studio and visual signing in an unpacked browser (PDF engine checks use real generated PDF files), signing on real customer forms, image compression, downloads rename/sort rules, blocker schedules and alarms over real time, keyboard shortcuts, right-click menus, desktop notifications, the login switcher, and the price and page monitors over several hours.
